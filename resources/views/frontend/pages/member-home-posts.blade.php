...
@forelse($posts as $key=>$post)
    @if ($post->getTable() == 'shares')
        <?php $share = $post; ?>
        <input id="member_home_post_id_{{ $key }}" data-key={{ $key }} type="number"
            value="{{ $share->post_id }}" hidden>
        <input id="member_home_user_id_{{ $key }}" data-key={{ $key }} type="number"
            value="{{ $share->user_id }}" hidden>
        <div class="post_center_box" data-key={{ $key }}>
                ...
                <div class="shareLikeButtons">
                    @foreach ($share->likes as $like)
                        @if ($like->share_id == $share->id && $like->ce == 1 && $like->like == 1 && $like->user_id == auth()->user()->id)
                            ...
                            <a class="liked" member_home_post_id="{{ $share->id }}"
                                member_home_user_id="{{ $share->user_id }}" member_home_like_type="0"
                                style="cursor: pointer;"><b class="liked"> <i class='fa fa-thumbs-up'></i>
                                    Liked</b></a>
                        @endif
                    @endforeach
                    @if ($displayLike == false)
                        <a class="like_color" member_home_post_id="{{ $share->id }}"
                            member_home_user_id="{{ $share->user_id }}" member_home_like_type="0"> <i
            ...
            <div id="comment_share_{{ $key }}" data-key={{ $key }}
                ...
                <div class="row" style="background-color: wheat; padding: 10px;">
                    <div class="col-sm-8">
                        <input style="background-color: wheat; outline: none; width: 100%;" type="text"
                            name="comment" id="comment_share_value_{{ $key }}"
    ...
    @else
        ...
        <div class="post_center_box">
                ...
                <div class="likeButtons">
                    @foreach ($post->likes as $like)
                        @if ($like->post_id == $post->id && $like->ce == 1 && $like->like == 1 && $like->user_id == auth()->user()->id)
                            <?php $displayLike = true; ?>
                            <a class="liked" member_home_post_id="{{ $post->id }}"
                                member_home_user_id="{{ $post->user->id }}" member_home_like_type="1"
                                style="cursor: pointer;"><b class="liked"><i class="fa fa-thumbs-up"
                                        aria-hidden="true"></i> Liked</b></a>
                        @endif
                    @endforeach
                    @if ($displayLike == false)
                        <a class="like_color" member_home_post_id="{{ $post->id }}"
                            member_home_user_id="{{ $post->user->id }}" member_home_like_type="1"><i
            ...
            <div id="comment_{{ $key }}" data-key={{ $key }}
                style="display: none; margin-top: 20px;">
                ...
                <div class="row" style="background-color: wheat; padding: 10px;">
                    <div class="col-sm-8">
                        <input style="background-color: wheat; outline: none; width: 100%;" type="text"
                            name="comment" id="comment_value_{{ $key }}" placeholder="Type comment here" />
                    </div>
